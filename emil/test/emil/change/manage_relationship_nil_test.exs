...
defmodule ThisTest do
  ...
  test "manage_relationship results from passing a map argument", %{post: post} do
    assert post = Changeset.for_update(post, :append_comment) |> Ash.update!()
  ...
  test "manage_relationship results from passing an array of maps argument", %{post: post} do
    assert post = Changeset.for_update(post, :append_comments) |> Ash.update!()
    assert %{comments: %Ash.NotLoaded{}} = post

    params = %{comments: nil}
    assert post = Changeset.for_update(post, :append_comments, params) |> Ash.update!()
    assert %{comments: []} = post
