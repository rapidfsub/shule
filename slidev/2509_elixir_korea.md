...
* JupyterLab, Google Colab, Apache Zeppelin
* 동시편집 가능
* 재현가능성
* https://livebook.dev/
...
많이들 써보셨을 JupyterLab, Google Colab 등과 비슷한 제품입니다.\
...
"기존에 쓰던 솔루션이 있으니, 데이터를 마이그레이션 해주세요."\
"기존 솔루션에서 관리하던 고객사가 수 백개라 일일히 수동으로 입력하기는 힘들어요."\
이 고객은 기존에 쓰던 솔루션이 있으니 우리 제품으로 옮기기는 쉽지 않을 겁니다.\
혹시 모르죠. 데이터 마이그레이션이 쉽게 된다면 써주실지도요.
-->
...
이거 개발하는 동안 고객이 될 뻔 했던 회사는 떠나갈 겁니다.\
게다가 우리는 코어 기능개발은 멈추고 대량등록 기능에만 많은 시간을 쓰게 되겠죠.
-->
...
그 외에도 무궁무진한 방법이 있겠죠.\
하지만 다들 저마다의 약점을 가지고 있는게 보이실겁니다.\
...
<!--
이제 엘릭서와 라이브북을 사용하면 어떻게 달라지는 지 보겠습니다.\
라이브북을 실행하고 attached node기능을 사용하여 실서버에 접속하여 코드를 실행하기만 하면 됩니다.\
